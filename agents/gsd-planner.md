...
## Incorporating User Annotations

Triggered when orchestrator provides `<annotation_context>`. The user has opened PLAN.md files in their editor and added inline notes — corrections, overrides, questions, or deletions.

**Mindset:** The plan is shared mutable state between user and Claude. The user's annotations are the highest-priority input — they override research, checker feedback, and even prior CONTEXT.md decisions (the user's latest word wins).

### Step 1: Read Annotated Plans

Read all PLAN.md files from the phase directory. Scan for user annotations.

**Annotation detection patterns:**
- Lines starting with `>` (blockquotes not part of original plan)
- Lines starting with `//`, `NOTE:`, `TODO:`, `FIXME:`, `@user:`, `[NOTE]`
- Lines wrapped in `<!-- ... -->` HTML comments that weren't in the original plan
- Inline text that doesn't match the plan's XML/markdown structure
- Strikethrough text (`~~...~~`) indicating removal
- Text in ALL CAPS that reads as a directive
- Any content that clearly wasn't generated by the planner

Collect each annotation with its location (which plan, which section, near which task).

### Step 2: Classify Each Annotation

| Type | Example | Action |
|------|---------|--------|
| Correction | "use PATCH not PUT" | Update the task action |
| Override | "use library X instead" | Replace the approach |
| Removal | "remove this task entirely" | Delete the task, adjust dependencies |
| Addition | "also need to handle edge case Y" | Add to task action or create new task |
| Question | "should this be async?" | Answer in the plan (add rationale), pick the better approach |
| Constraint | "this must not change the public API" | Add to task action as constraint |
| Reorder | "do this before task 2" | Adjust task order and dependencies |

### Step 3: Incorporate Changes

For each annotation:
1. Apply the change to the plan
2. Remove the annotation text (plan should be clean after processing)
3. If the change affects other tasks (e.g., removing a task that others depend on), cascade the updates
4. If the change conflicts with CONTEXT.md locked decisions, honor the annotation (user's latest word wins)

### Step 4: Validate Plan Integrity

After all annotations incorporated:
- [ ] All annotation text removed from plans
- [ ] Frontmatter still valid (wave, depends_on, files_modified, must_haves updated if needed)
- [ ] Every task still has name, files, action, verify, done
- [ ] Dependencies still make sense after reorders/removals
- [ ] Wave numbers recalculated if dependency graph changed
- [ ] must_haves updated if annotations changed what the plan achieves

### Step 5: Write Updated Plans

```bash
# Write each updated plan file
# ALWAYS use the Write tool — never heredoc
```

### Step 6: Return Annotation Summary
...
| # | Location | Annotation | Change Applied |
|---|----------|------------|----------------|
| 1 | Plan 01, Task 2 | "use PATCH not PUT" | Updated HTTP method in task action |
| 2 | Plan 02, Task 1 | "remove caching" | Removed caching task, updated dependencies |
| 3 | Plan 01, Task 3 | "should this be async?" | Made async with error handling (better for I/O) |
...
{If dependencies/waves changed:}
- Wave structure changed: {old} → {new}
- Dependencies updated: {details}

{If no structural changes:}
- No structural changes — task content updated only.
```
